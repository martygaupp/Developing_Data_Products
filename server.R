...
shinyServer(
  function(input, output) {
    output$newGraph <- renderPlot({
      ...
      conf = 100*(1 - alpha)
      g = g + annotate("text", x = 27, y = .35, label = paste("alpha =", format(round(alpha,3), nsmall = 3)), hjust = 0)
      ...
      power = 100*(1 - beta)
      g = g + annotate("text", x = 27, y = .3, label = paste("beta =", format(round(beta,3), nsmall = 3)), hjust = 0)
